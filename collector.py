...
POWER_EVENT_PATTERN = re.compile(
    r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} [+-]\d{4})\s+(Sleep|Wake|DarkWake)\b(?!\s*Requests\b)(.*)"
)
...
def main():
    ...
    parser.add_argument(
        "--interval",
        type=int,
        default=cfg["polling_interval_seconds"],
        help=f"Polling interval in seconds (default: {cfg['polling_interval_seconds']})",
    )
